...
import { getSiteKey, getCurrentYear } from "../../lib/site";
...
const YearSelector: FunctionComponent = ({}) => {
	...
	const config = require(`../../../_db/${getSiteKey()}/config.json`);
	...
	let currentValue = params.year ?? getCurrentYear();
	...
	const onChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
		...
		if(event.target.value == getCurrentYear()){
			router.push('/');
		} else {
